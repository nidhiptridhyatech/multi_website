...
class VoyagerUserController extends VoyagerBaseController
{
    ...
    {
        $userwiseLandingpageDetail = UserwiseLandingpageDetail::where('user_id',$id)->pluck('landingpage_type_id')->toArray();
        
        foreach ($request->user_belongstomany_landingpage_type_relationship as $kk => $vv){
            if(! in_array($vv, $userwiseLandingpageDetail)){
                $userwiseLandingpageDetails = new UserwiseLandingpageDetail();
                $userwiseLandingpageDetails->user_id = $id;
                $userwiseLandingpageDetails->landingpage_type_id = $vv;
                $userwiseLandingpageDetails->save();
            }
        }
        foreach($userwiseLandingpageDetail as $kk => $vv){
            if(! in_array($vv, $request->user_belongstomany_landingpage_type_relationship)){
                UserwiseLandingpageDetail::Where("user_id",$id)->Where("landingpage_type_id",$vv)->delete();
            }
        }
        ...
        $currentUserEmail = User::where("id",$id)->first();
        $this->editRules($request,$id);
    ...
    public function store(Request $request)
    {
        ...
        foreach ($request->user_belongstomany_landingpage_type_relationship as $key => $value) {
            $userwiseLandingpageDetail = new UserwiseLandingpageDetail();
            $userwiseLandingpageDetail->user_id = $data->id;
            $userwiseLandingpageDetail->landingpage_type_id = $value;
            $userwiseLandingpageDetail->save();
        }
        

        $eData =array('name'=>$data->name,"email"=>$data->email,"password"=>$request->password);
        Mail::send(['text'=>'mail'], $eData, function($message) use ($data){
            $message->to($data->email, $data->name)->subject
            ('Account Created');
            $message->from('phptesting2018@gmail.com','Multi Website');
        });
    ...
    public static function addRules(Request $request) {
    ...
    public static function editRules(Request $request,$email) {
        return $request->validate([
            'email' => 'unique:users,email,'.$email.',id'
        ]);
    }
}
